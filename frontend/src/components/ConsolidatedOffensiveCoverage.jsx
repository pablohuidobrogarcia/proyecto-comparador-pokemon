import PokemonSprite from './PokemonSprite';
import { moveEffectiveness } from '../utils/moveEffectiveness';
import { typeColor } from '../utils/typeColors';
import typeChart from '../data/type-chart.json';

const TYPE_ORDER = Object.keys(typeChart);

function fmt(n) {
  if (Number.isInteger(n)) return String(n);
  return n.toFixed(3).replace(/0+$/, '').replace(/\.$/, '');
}

function multLabel(mult) {
  if (mult === 0) return { text: '0', cls: 'cov-zero' };
  if (mult === 0.25) return { text: '¼', cls: 'cov-resist' };
  if (mult === 0.5) return { text: '½', cls: 'cov-resist' };
  if (mult === 1) return { text: '', cls: '' };
  if (mult > 1) return { text: `${fmt(mult)}×`, cls: 'cov-weak' };
  return { text: fmt(mult), cls: 'cov-resist' };
}

export default function ConsolidatedOffensiveCoverage({ pokemon, moveSelections, colors }) {
  const active = pokemon.filter((p) => {
    const moves = moveSelections?.[p.id] || [];
    return moves.some((m) => m.esDaño);
  });

  if (!active.length) return null;

  const bestForPokemon = active.map((p) => {
    const damageMoves = (moveSelections?.[p.id] || []).filter((m) => m.esDaño);
    return {
      pokemon: p,
      bestByType: TYPE_ORDER.map((defender) =>
        Math.max(...damageMoves.map((m) => moveEffectiveness(m.tipo, m.name, [defender])))
      ),
    };
  });

  return (
    <section className="table-section coverage-section offensive-section">
      <h2>Cobertura ofensiva del equipo</h2>
      <div className="table-wrapper">
        <table className="coverage-table offensive-table">
          <thead>
            <tr>
              <th className="cov-attr">Defensor ↓</th>
              {bestForPokemon.map(({ pokemon }, pi) => (
                <th key={pokemon.id}>
                  <div className="cov-poke">
                    {pokemon.sprite && <PokemonSprite pokemon={pokemon} alt="" />}
                    <span style={{ color: colors[pi % colors.length] }}>{pokemon.name}</span>
                  </div>
                </th>
              ))}
              <th>Mejor cobertura del equipo</th>
            </tr>
          </thead>
          <tbody>
            {TYPE_ORDER.map((defender, r) => {
              const teamBest = Math.max(...bestForPokemon.map((b) => b.bestByType[r]));
              const teamBestLabel = multLabel(teamBest);
              return (
                <tr key={defender} className={r % 2 === 1 ? 'cov-even' : 'cov-odd'}>
                  <td className="cov-type-cell">
                    <span
                      className="type-badge cov-type-badge"
                      style={{ backgroundColor: typeColor(defender) }}
                    >
                      {defender}
                    </span>
                  </td>
                  {bestForPokemon.map((b) => {
                    const { text, cls } = multLabel(b.bestByType[r]);
                    return (
                      <td key={b.pokemon.id} className={cls}>
                        {text}
                      </td>
                    );
                  })}
                  <td className={teamBestLabel.cls}>{teamBestLabel.text}</td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </section>
  );
}
